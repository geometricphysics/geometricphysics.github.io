define(function(require, exports, module)
{
    ...
    pythonLanguage = require('./pythonServices').Language;
    Services = require('./pythonServices').Services;
    ...
    var ScriptInfo = (function () {
        ...
        ScriptInfo.prototype.editContent = function (minChar, limChar, newText) {
            ...
            this.editRanges.push({
                ...
                editRange: new pythonLanguage.ScriptEditRange(minChar, limChar, (limChar - minChar) + newText.length)
            });
        ...
        ScriptInfo.prototype.getEditRangeSinceVersion = function (version) {
            ...
            if(initialEditRangeIndex < 0 || initialEditRangeIndex >= this.editRanges.length) {
                return pythonLanguage.ScriptEditRange.unknown();
            }
            ...
            return new pythonLanguage.ScriptEditRange(minDistFromStart, entries[0].length - minDistFromEnd, aggDelta);
        };
        return ScriptInfo;
    })();
    ...
    var ScriptManager = (function ()
    {
        ...
        {
            console.log('ScriptManager.updateScript(' + JSON.stringify({'name': name,'isResident': isResident}) + ')');
        ...
        ScriptManager.prototype.getLanguageService = function () {
            var ls = new Services.ServicesFactory().createLanguageServiceShim(this);
            this.ls = ls;
            return ls;
        };
        ScriptManager.prototype.parseSourceText = function (fileName, sourceText) {
            var parser = new pythonLanguage.Parser();
        ...
        ScriptManager.prototype.parseFile = function (fileName) {
            var sourceText = new pythonLanguage.StringSourceText(IO.readFile(fileName));
        ...
        ScriptManager.prototype.lineColToPosition = function (fileName, line, col) {
            ...
            return pythonLanguage.getPositionFromLineColumn(script, line, col);
        };
        ScriptManager.prototype.positionToLineCol = function (fileName, position) {
            var script = this.ls.languageService.getScriptAST(fileName);
            assert.notNull(script);
            var result = pythonLanguage.getLineColumnFromPosition(script, position);
